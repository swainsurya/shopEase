    ...
    <section className="py-6">
      ...
      <div className="flex flex-1 items-center gap-3 overflow-x-scroll overflow-y-hidden noScroll_ch noscroll_all">
        {products.map((product) => (
          <Card key={product.id} className="text-center max-h-[50px] w-fit flex items-center cursor-pointer">
            <CardContent className="w-full p-4">
              <p className="font-medium text-blue-900 text-xl">{product.category}</p>
            </CardContent>
          </Card>
        ))}
      </div>
